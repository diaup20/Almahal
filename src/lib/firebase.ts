...
import { getFirestore, setLogLevel, getDoc as firestoreGetDoc, getDocs as firestoreGetDocs, DocumentReference, Query, DocumentData } from 'firebase/firestore';
...
// Suppress verbose internal firestore logs in sandboxed/offline environments
try {
  setLogLevel('silent');
} catch {
  // Ignore
...
export async function safeGetDoc<T = DocumentData>(docRef: DocumentReference<T>, timeoutMs = 2000) {
  try {
...
export async function safeGetDocs<T = DocumentData>(queryRef: Query<T>, timeoutMs = 2000) {
  try {
    const timeoutPromise = new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error('Firestore operation timed out')), timeoutMs)
    );
